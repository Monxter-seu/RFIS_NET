...
from g_mlp import mixNet,maskNet,dayShiftNet
from pit_criterion import new_loss,variance_loss
from data import MyDataLoader, MyDataset
from conv_tasnet import TemporalConvNet
from customLoader import CustomDataset,NewCustomDataset
...
if __name__ == "__main__":
    ...
    # 定义损失函数和优化器
    #criterion = variance_loss
    optimizer = optim.Adam(model.parameters(), lr=0.00001)

    train_dataset = NewCustomDataset('D:\\frequencyProcess\\testNew\\tr\\mix')
    ...
    test_dataset = NewCustomDataset('D:\\frequencyProcess\\testNew\\tt\\mix')
    ...
    for epoch in range(num_epochs):
        ...
        for data, left_label in tqdm(train_loader):
            data = data.cuda()
            left_label = left_label.cuda().float()
            output1, outputs0 = model(data)
            loss = variance_loss(output1, outputs0, left_label)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    left_macro_f1_scores = []
    with torch.no_grad():
        for data, left_label in tqdm(test_loader):
            data = data.cuda()
            left_label = left_label.cuda().float()
            outputs1, outputs0 = model(data)
            loss = variance_loss(outputs1, outputs0, left_label)
        ...
        torch.save(model.state_dict(), "representer.pth")
        print('==========')
