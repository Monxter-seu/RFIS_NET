...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for data, left_label, right_label in tqdm(train_loader):
            ...
            outputs0,outputs1 = model(data)
            loss = new_loss(outputs0,outputs1,left_label,right_label,1)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        train_accuracy1 = train_correct1 / len(train_loader.dataset)
        print('============')
        ...
        with torch.no_grad():
            ...
            cv_accuracy1 = cv_correct1 / len(cv_loader.dataset)
            left_average_micro_f1 = np.mean(left_micro_f1_scores)
            left_average_macro_f1 = np.mean(left_macro_f1_scores)
            right_average_micro_f1 = np.mean(right_micro_f1_scores)
            right_average_macro_f1 = np.mean(right_macro_f1_scores)
    ...
    with torch.no_grad():
        ...
        torch.save(model.state_dict(), "confounder.pth")
        print('==========')
