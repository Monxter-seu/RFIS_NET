    ...
    for epoch in range(num_epochs):
        ...
        for data, left_label, right_label in tqdm(train_loader):
            for confounder in all_confouders:
                ...
                outputs0,outputs1,outputs2 = model(data,confounder)
                loss = confounder_loss(outputs0,outputs1,outputs2,left_label,right_label,1,1)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        train_loss /= (4*len(train_loader.dataset))
        train_accuracy0 = train_correct0 / (4*len(train_loader.dataset))
        train_accuracy1 = train_correct1 / (4*len(train_loader.dataset))
        train_accuracy2 = train_correct2 / (4*len(train_loader.dataset))
        print('============')
    ...
    with torch.no_grad():
        for data, left_label, right_label in tqdm(test_loader):
            for confounder in all_confouders:
                ...
                right_macro_f1 = f1_score(predicted1.cpu(), right_label.cpu(), average='macro')
                right_micro_f1 = f1_score(predicted1.cpu(), right_label.cpu(), average='micro')
                tt_correct2 += (predicted2 == left_label.unsqueeze(1)).sum().item()
                confounder_micro_f1 = f1_score(predicted2.cpu(), left_label.unsqueeze(1).cpu(), average='micro')
                confounder_macro_f1 = f1_score(predicted2.cpu(), left_label.unsqueeze(1).cpu(), average='macro')
        ...
        tt_loss /= (4*len(test_loader.dataset))
        tt_accuracy0 = tt_correct0 / (4*len(test_loader.dataset))
        tt_accuracy1 = tt_correct1 / (4*len(test_loader.dataset))
